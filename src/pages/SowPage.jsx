          ...
          {tab==='info' && (
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              {sow.cull_date ? (
                ...
                <div style={{background:'#FDECEA',borderRadius:10,padding:12}}>
                  <div style={{fontSize:11,color:'#C0392B',fontWeight:600,marginBottom:8}}>⚫ 도태 등록</div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
                    <DateInput label="도태일" value={info.cull_date||''} onChange={v=>setInfo(p=>({...p,cull_date:v,status:'도태'}))}/>
                    <div style={{display:'flex',flexDirection:'column',gap:4}}>
                      <label style={{fontSize:11,color:'#888'}}>도태사유</label>
                      <select value={info.cull_reason||''} onChange={e=>setInfo(p=>({...p,cull_reason:e.target.value}))}
                        ...
                        <option value=''>선택</option>
                        {['저능력','지제','번식장애','노령','폐사','기타'].map(r=><option key={r}>{r}</option>)}
                      </select>
                    </div>
                  </div>
                  <div style={{fontSize:11,color:'#C0392B'}}>도태일 입력 후 저장하면 목록 맨 아래로 이동돼요</div>
                </div>
              )}
            </div>
          )}
