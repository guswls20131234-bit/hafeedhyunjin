...
function SowModal({ sow, cycles, onClose, onSave, onDelete, farmSlug }) {
  ...
  async function handleCull() {
    ...
    if (!cullDate) return
    setSaving(true)
    await onSave({ ...info, cull_date: cullDate, cull_reason: cullReason, status: '도태' }, cycle)
    setSaving(false)
    onClose()
  }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'flex-end',justifyContent:'center'}}
      ...
      <div style={{background:'white',borderRadius:'16px 16px 0 0',width:'100%',maxWidth:480,maxHeight:'90vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
...
export default function SowPage({ farmSlug }) {
  ...
  // 저장
  async function handleSave(info, cycle) {
    // 1. 모돈 기본정보 저장
    ...
    if (error || !savedSow) return

    // 2. 사이클 저장
    if (cycle && cycle.parity) {
      const existing = cycles.find(c => c.sow_record_id === savedSow.id && c.parity === cycle.parity)
      if (existing) {
        await supabase.from('sow_cycles').update({ ...cycle, sow_record_id: savedSow.id, farm_slug: farmSlug })
          .eq('id', existing.id)
      } else if (cycle.mating_date || cycle.farrow_date) {
        await supabase.from('sow_cycles').insert({ ...cycle, sow_record_id: savedSow.id, farm_slug: farmSlug })
      }
    }
    await load()
  }
